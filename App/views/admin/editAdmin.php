...
// Ambil data dari $old (kalau POST gagal) atau dari $admin (data asli)
$idAdmin      = isset($admin['id_account']) ? (int)$admin['id_account'] : (int)($old['id_account'] ?? 0);
$nama         = $old['nama']         ?? ($admin['nama']         ?? '');
$email        = $old['email']        ?? ($admin['email']        ?? '');
$nim_nip      = $old['nim_nip']      ?? ($admin['nim_nip']      ?? '');
$role         = $old['role']         ?? ($admin['role']         ?? 'admin');
$status_aktif = $old['status_aktif'] ?? ($admin['status_aktif'] ?? 'aktif');
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    <meta charset="UTF-8">
    <title>Edit Admin | Kubooking</title>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <div class="px-4 sm:px-8 pb-10 max-w-3xl mx-auto w-full space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-[#1e3a5f]">Edit Admin</h1>
                    <p class="text-sm text-gray-500 mt-1">
                        Ubah informasi akun admin atau super admin.
                    </p>
                </div>
            ...
            <div class="bg-white shadow rounded-xl p-6 sm:p-8">
                <form action="index.php?controller=superAdmin&action=updateAdmin" method="POST" class="space-y-4">
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                Isi hanya jika ingin mengganti password. Minimal 8 karakter, kombinasi huruf dan angka.
                            </p>
                        </div>
