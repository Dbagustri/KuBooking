...
$makeUrl = fn($p) => htmlspecialchars(kubooking_pagination_build_url($params, $pageKey, $p));
...
// Kalau totalPages < 3, otomatis aman karena min/max
...
<div class="mt-5 flex flex-col sm:flex-row items-center justify-between gap-3">
    <div class="text-sm text-slate-600">
        Halaman <span class="font-semibold text-slate-800"><?= $currentPage ?></span>
        dari <span class="font-semibold text-slate-800"><?= $totalPages ?></span>
    </div>
    ...
    <nav class="flex items-center gap-2" aria-label="Pagination">
        ...
        <?php if ($currentPage > 1): ?>
            <a class="<?= $btnBase ?> <?= $btnNormal ?>"
                href="<?= $makeUrl($currentPage - 1) ?>" aria-label="Previous page">
                Prev
            </a>
        <?php else: ?>
        ...
        <!-- Left ellipsis (kalau masih ada page sebelum start) -->
        <?php if ($start > 1): ?>
        ...
        <?php for ($p = $start; $p <= $end; $p++): ?>
            <a class="<?= $btnBase ?> <?= $p === $currentPage ? $btnActive : $btnNormal ?>"
                href="<?= $makeUrl($p) ?>" <?= $p === $currentPage ? 'aria-current="page"' : '' ?>>
                <?= $p ?>
            </a>
        <?php endfor; ?>

        <!-- Right ellipsis (kalau masih ada page setelah end) -->
        ...
        <?php if ($currentPage < $totalPages): ?>
            <a class="<?= $btnBase ?> <?= $btnNormal ?>"
                href="<?= $makeUrl($totalPages) ?>" aria-label="Last page">
                Last
            </a>
        <?php else: ?>
