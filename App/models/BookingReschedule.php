...
class BookingReschedule extends Model
{
     ...
     * Sekarang juga mengisi join_reschedule_until (window join anggota via kode).
     */
    public function createRequest(array $data, array $memberIds = []): int
    {
        $sql = "INSERT INTO " . self::$table . "
                (id_bookings, id_ruangan, id_user,
                 new_start_time, new_end_time, new_tanggal, alasan,
        ...
        $stmt->execute([
            ...
            // window join anggota reschedule (misal 10 menit)
        ...
        // Copy anggota awal (misal dari Booking_member)
        if (!empty($memberIds)) {
    ...
    /**
     * Replace semua member draft reschedule
     */
    ...
    /**
     * Copy anggota lama booking → draft reschedule
     */
     ...
     * Dipakai saat user join pakai KODE KELOMPOK setelah reschedule.
     */
